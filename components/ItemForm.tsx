...
export function ItemForm({ onSubmitSuccess }: ItemFormProps) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors }
  } = useForm<z.infer<typeof formSchema>>(formResolver);
  ...
  return (
    <Form.Root
      ...
      className="bg-gray-500 p-6 rounded-md"
    >
      <Form.Field name="name">
        <div className="flex flex-col mb-4">
          ...
          {errors.name && (
            <Form.Message className="error-class text-red-500">
              {errors.name.message}
            </Form.Message>
          )}
        </div>
